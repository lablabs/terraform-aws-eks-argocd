...
| <a name="input_helm_set_sensitive"></a> [helm\_set\_sensitive](#input\_helm\_set\_sensitive) | Value block with custom sensitive values to be merged with the values yaml that won't be exposed in the plan's diff. | `map(any)` |
...
| <a name="input_self_managed_helm_upgrade_install"></a> [self\_managed\_helm\_upgrade\_install](#input\_self\_managed\_helm\_upgrade\_install) | Set to true to enable Helm upgrade with --install flag for self-managed installation. This will install the release if it doesn't exist or upgrade it if it does. | `bool` |
